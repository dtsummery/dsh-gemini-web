...
window.__ModuleLoader__.load({
  ...
  factory: (require) => {
    ...
    var CSS = [
      ...
      ".gws-row-key{flex:none;width:48px;color:var(--dsw-alias-label-secondary);font-size:12px}",
      ...
      ".gws-card-head{display:flex;align-items:center;justify-content:space-between;gap:10px;flex-wrap:wrap}",
      ".gws-card-actions{display:flex;gap:6px}",
      ".gws-btn--sm{height:26px;padding:0 10px;font-size:12px;border-radius:8px}",
      ...
      ".gws-chip{font-family:ui-monospace,SFMono-Regular,Menlo,monospace;font-size:11px;padding:3px 8px;border-radius:7px;background:var(--dsw-alias-bg-layer-3);color:var(--dsw-alias-label-secondary)}",
    ...
    function keepAliveRow(data) {
      ...
      var intervalText = ka.intervalMinutes < 1
        ? Math.round(ka.intervalMinutes * 60) + " 秒"
        : ka.intervalMinutes + " 分钟"
      var text = (ka.running ? "运行中" : "待命") + " · 每 " + intervalText + "检查 · 已回写 " + (ka.pushes || 0) + " 次"
      var note = ka.lastError || (ka.lastPushAt
        ? "最近回写 " + clock(ka.lastPushAt) + " · 最近轮转 " + clock(ka.lastRotateAt)
        : "尚未回写过：Google 还没换发新登录票")
    ...
    function GeminiWebPage(props) {
      ...
      function saveModels() {
        ...
        if (list.length === 0) {
          setLocal({ busy: "", message: "", error: "至少要勾选一个模型" })
          return
        }
      ...
      var serverModelIds = modelOptionsList.filter(function (option) { return option.selected === true }).map(function (option) { return option.id })
      var shownModelIds = modelDraft === null ? serverModelIds : modelDraft
      ...
      var opening = !!(data && data.login && data.login.phase === "opening")
      var disabled = busy !== ""
      ...
      return h(
        ...
        h(
          ...
          h(
            ...
            h(StatusRow, {
              label: "账号",
              tone: account ? (account.health === "ok" ? "ok" : "warn") : "idle",
              text: account
                ? "#" + account.id + " · " + (account.health === "ok" ? "登录态正常" : "健康度 " + account.health)
                : "未登录",
              note: account ? (account.hasPsidts ? "含 __Secure-1PSIDTS" : "缺 __Secure-1PSIDTS，需重新登录") : ""
            })
          )
        ),
        ...
        h(
          ...
          h(
            "div",
            { className: "gws-card-head" },
            h("span", { className: "gws-hint" }, "已选 " + shownModelIds.length + "/" + modelOptionsList.length + " 个：保存后会话窗口的模型选择器即可选这些模型"),
            h(
              "div",
              { className: "gws-card-actions" },
              h(Button, {
                variant: "ghost",
                disabled: disabled || !config.writable || modelOptionsList.length === 0,
                onClick: function () { setModelDraft(modelOptionsList.map(function (option) { return option.id })) }
              }, "全选"),
              h(Button, {
                variant: "ghost",
                disabled: disabled || !config.writable,
                onClick: function () { setModelDraft(["gemini-3.8-flash", "gemini-3.1-pro"]) }
              }, "默认"),
              h(Button, {
                variant: "ghost",
                disabled: disabled || modelDraft === null,
                onClick: function () { setModelDraft(null) }
              }, "撤销")
            )
          ),
          modelOptionsList.length === 0
            ? h("p", { className: "gws-hint" }, "读不到模型清单：刚更新过插件的话，请重启 DSH Desktop 让 host 端加载新版本（刷新页面只更新浏览器端）；否则先点下面的「重启内核」。")
            : h(
              "div",
              ...
              modelOptionsList.map(function (option) {
                return h(
                  "label",
                  { className: "gws-model", key: option.id },
                  h("input", {
                    type: "checkbox",
                    checked: shownModelIds.indexOf(option.id) >= 0,
                    disabled: !config.writable || disabled,
                    onChange: function () { toggleModel(option.id) }
                  }),
                  h("span", { className: "gws-model-name", title: option.name + "（" + option.id + "）" }, option.name),
                  option.live === true ? null : h("span", { className: "gws-model-flag" }, "未解锁"),
          ...
          h(
            ...
            h(
              Button,
              { variant: "primary", disabled: disabled || !config.writable || !modelDirty, onClick: saveModels },
              busy === "models" ? "保存中…" : "保存模型选择"
            ),
            modelDirty ? h("span", { className: "gws-hint" }, "有未保存的改动") : null
          )
        ),
        ...
        h(
          ...
          h(
            ...
            h(Field, {
              label: "保活检查间隔（分钟，可填 0.25 即 15 秒）",
              type: "number",
              min: 0.1,
              max: 120,
              step: "any",
              value: String(value.keepAliveIntervalMinutes === undefined ? 0.25 : value.keepAliveIntervalMinutes),
