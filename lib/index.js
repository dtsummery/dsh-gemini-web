...
export function apply(ctx, entryConfig = {}) {
  ...
  /** 登录票超过这么久没变化，就重启保活浏览器催一次轮转（实测浏览器 18–40 分钟换发一次）。 */
  const STALE_ROTATE_MS = 45 * 60 * 1000
  ...
  const keepAliveTick = async (force = false) => {
    ...
    try {
      ...
      if (fingerprint === keepAliveState.fingerprint) {
        // 票长时间不动：重启一次保活浏览器，靠页面重载重新走 Google 的会话流程催轮转。
  ...
  const adapterDeps = {
    ...
    onAuthFailure: () => { void keepAliveTick(true) },
    log: (msg) => log(msg)
  }
