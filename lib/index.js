...
import { GeminiWebAdapter, DEFAULT_MODELS, MODEL_CATALOG } from './adapter.js'
...
const Config = z.object({
  ...
  /** 保活检查间隔（分钟）：每次检查读一次浏览器 cookie（很轻），票变了才回写内核。
   *  默认 0.25 分钟（15 秒）——Google 轮转后旧票立即失效，间隔直接等于失败窗口（浏览器实测 9–18 分钟轮转一次）。 */
  keepAliveIntervalMinutes: z.number().default(0.25),
  /** 暴露给 DSH 的模型 id 列表：只把这些模型放进会话窗口的模型选择器。 */
  models: z.array(z.string()).default(['gemini-3.8-flash', 'gemini-3.1-pro'])
})
 ...
 * @returns {{enabled: boolean, kernelPath: string, port: number, proxyUrl: string, browserPath: string, exposeLimitationNotice: boolean, keepAliveEnabled: boolean, keepAliveIntervalMinutes: number, models: string[]}}
 */
function normalizeConfig(...sources) {
  ...
  return {
    ...
    keepAliveIntervalMinutes: Number.isFinite(keepAliveMinutes) ? Math.min(120, Math.max(0.1, keepAliveMinutes)) : 0.25,
...
export function apply(ctx, entryConfig = {}) {
  ...
  /** 登录票超过这么久没变化，就重启保活浏览器催一次轮转（实测浏览器 9–18 分钟换发一次）。 */
  const STALE_ROTATE_MS = 25 * 60 * 1000
  const keepAliveState = {
    busy: false,
    lastCheckAt: 0,
    lastPushAt: 0,
    lastVerifyAt: 0,
    // 内核报过「登录票不可用」时置位：若浏览器也没换票，就重启浏览器催票。
    forceRotate: false,
    lastForceRotateAt: 0,
    // 登录票最后一次变化的时刻。长时间不动说明浏览器侧也没在轮转，需要催一次。
  ...
  /**
   * 清掉占着插件 profile 的残留 headless 保活实例（不动可见的登录窗口）。
   * 登录前必须做这一步：profile 被 headless 占着时，新的登录窗口会被 Edge 的单实例
   * 机制顶掉 —— 表现就是「点登录、窗口一闪就没了」。
   */
  const killStaleHeadlessBrowsers = async () => {
    const probe = new KeepAliveBrowser({
      browserPath: currentConfig().browserPath,
      profileDir: join(dir, 'browser-profile')
    })
    await probe.killStaleProfileProcesses().catch(() => {})
  }
  ...
  const startLogin = async () => {
    ...
    if (loginState.phase === 'opening') return loginState
    // 先占住「登录中」这个状态，再让保活退场：保活 tick 是 15 秒一轮，
    // 顺序反了它就会在窗口打开前把 profile 抢回去，登录窗口一开就被顶掉。
    loginState = { phase: 'opening', message: '正在打开登录窗口…', at: Date.now() }
    await stopKeepAliveBrowser()
    await killStaleHeadlessBrowsers()
  ...
  const confirmLogin = async () => {
    ...
    try {
      ...
      await browser.close().catch(() => {})
      browser = null
      // 登录态刚写好就立刻跑一轮保活，别让内核空等一个定时周期。
      void keepAliveTick(true)
    } catch (error) {
   ...
   */
  const keepAliveTick = async (force = false) => {
    ...
    // 登录流程与保活浏览器共用同一个 profile：登录期间必须让路，
    // 否则保活一启动就会把用户的登录窗口顶掉（窗口闪退）。
    if (loginState.phase === 'opening' || loginState.phase === 'awaiting-confirm' || loginState.phase === 'reading') {
      // 窗口被用户自己关掉、或打开卡住超过 1 分钟，就放行，别把保活永久卡死。
      const closed = loginState.phase === 'awaiting-confirm' && browser?.loginWindowOpen !== true
      const stuck = loginState.phase === 'opening' && Date.now() - loginState.at > 60000
      if (closed || stuck) {
        loginState = {
          phase: 'idle',
          message: closed ? '登录窗口已关闭，未完成登录' : '登录窗口打开失败，请重试',
          at: Date.now()
        }
      } else {
        return { pushed: false, message: '登录窗口使用中，保活暂时停用' }
      }
    }
    // 定时器 30 秒醒一次，启动浏览器可能要几秒到几十秒：加锁避免上一轮没结束就重入。
    if (keepAliveState.busy) return { pushed: false, message: '上一次保活检查还没结束' }
    const intervalMs = Math.max(0.1, currentConfig().keepAliveIntervalMinutes) * 60000
    if (!force && Date.now() - keepAliveState.lastCheckAt < intervalMs) {
    ...
    try {
      ...
      // 每半小时顺手验一次内核账号：票没换、但内核已失效，说明浏览器自己也拿不到新票，
      // 只能重新登录，界面要把这句话说清楚，别让它看起来像随机掉线。
      if (Date.now() - keepAliveState.lastVerifyAt > 30 * 60 * 1000) {
        keepAliveState.lastVerifyAt = Date.now()
        const check = await kernel.checkCookie(mine[0].id).catch(() => undefined)
        if (check && check.ok === false) {
          keepAliveState.lastError = `内核登录态已失效（${check.detail ?? check.error ?? '未知原因'}）；浏览器暂时也没换到新票，请点「登录 Google」重新登录`
        }
      }
      ...
      if (fingerprint === keepAliveState.fingerprint) {
        // 内核刚报过「登录票不可用」，而浏览器这张票还没换：说明浏览器自己卡住了
        // （headless 后台被节流、DBSC 刷新停摆），只能重启浏览器实例让它重新走一遍
        // 会话流程催票——页面 reload 是不够的，实测 reload 只会刷新 SIDCC 系列。
        if (keepAliveState.forceRotate && Date.now() - keepAliveState.lastForceRotateAt > 60000) {
          keepAliveState.forceRotate = false
          keepAliveState.lastForceRotateAt = Date.now()
          log('[keepalive] 内核报登录票失效、浏览器又没换票 —— 重启保活浏览器催一次')
          await stopKeepAliveBrowser()
          return { pushed: false, message: '登录票失效且浏览器未换票，已重启保活浏览器催 Google 换票，请重发一次' }
        }
        // 票长时间不动：同样重启一次保活浏览器催轮转。
        if (Date.now() - keepAliveState.lastRotateAt > STALE_ROTATE_MS) {
          keepAliveState.lastRotateAt = Date.now()
          log(`[keepalive] 登录票已 ${Math.round(STALE_ROTATE_MS / 60000)} 分钟没有更新，重启保活浏览器催一次轮转`)
          await stopKeepAliveBrowser()
          return { pushed: false, message: '登录票长时间未更新，已重启保活浏览器催轮转' }
        }
  ...
  const startKeepAliveLoop = () => {
    ...
    keepAliveTimer = setInterval(() => { void keepAliveTick() }, 5000)
    keepAliveTimer.unref?.()
    // 首次很快跑一轮：DSH 刚起来时内核里的登录票可能已经过期（关机期间没人续期），
    // 越早回写越好。
    setTimeout(() => { void keepAliveTick(true) }, 15000).unref?.()
  }

  /**
   * 内核存活看门狗。
   *
   * 适配器是拿着内核地址直接发请求的，不在请求前拉起内核；内核一旦崩溃或被外部杀掉，
   * 请求会一直失败到下一次手动操作（点「连通性测试」等）。这里每分钟探一次健康接口，
   * 探不到就重新拉起。
   */
  const startKernelWatchdog = () => {
    ...
    kernelWatchdog = setInterval(() => {
      void (async () => {
        if (disposed || !currentConfig().enabled) return
        if (kernel === null || starting !== null) return
        try {
          if (await kernel.health()) return
          log('[watchdog] 内核无响应，正在重新拉起')
          kernel = null
          await ensureKernel()
          log('[watchdog] 内核已重新就绪')
        } catch (error) {
          log(`[watchdog] 检查失败：${String(error?.message ?? error)}`)
        }
      })()
    }, 60000)
  ...
  const buildStatus = async () => {
    const status = {
      ...
      login: { ...loginState, windowOpen: browser?.loginWindowOpen === true },
      keepAlive: {
        ...
        lastRotateAt: keepAliveState.lastRotateAt,
    ...
    // 设置页的模型勾选表：列出全部可选项，标出「已选」与「内核当下真的提供」。
    const liveIds = status.kernel.models ?? []
    status.modelOptions = MODEL_CATALOG.map((m) => ({
      id: m.id,
      name: m.name,
      input: m.input,
      selected: currentConfig().models.includes(m.id),
      live: liveIds.includes(m.id)
    }))
    return status
  }
  ...
  const adapterDeps = {
    getBaseUrl: baseUrl,
    getApiKey: () => apiKey,
    getModels: () => currentConfig().models,
    // 内核报「登录票不可用」时立即催一轮保活，把浏览器里的新票回写内核（用户重发即可）。
    onAuthFailure: () => {
      // 内核报「登录票不可用」：置位催票标记并立刻检查一轮；
      // 若浏览器手里已经是新票就回写，若还是旧票就重启浏览器催。
      keepAliveState.forceRotate = true
      void keepAliveTick(true)
    },
    log: (msg) => log(msg)
  }
   ...
   * 之前把参数当值直接合并，函数会被 normalizeConfig 过滤掉，于是设置页里任何改动
   * 都进不了运行期配置 —— 默认值恰好等于默认配置，所以一直没暴露。
   */
  ...
  /** 按当前 config 应用运行时状态（幂等：注册与启动都有 guard）。 */
  const applyConfig = () => {
  ...
  ctx.inject(['webServer'], (sub) => {
    ...
    reg({
      ...
      handler: guard(async (_req, res) => {
        const result = await keepAliveTick(true)
        sendJson(res, 200, {
    ...
    reg({
      ...
      handler: guard(async (_req, res) => {
        ...
        // 插件没托管实例时也可能残留着上一轮的内核进程，按端口一并清掉。
        if (kernel !== null) {
          await kernel.shutdown().catch(() => {})
          kernel = null
        } else {
          const probe = new Kernel({
            binPath: target,
            port: kernelPort,
            dbPath: join(dir, 'kernel.db'),
            adminToken,
        ...
        try {
          ...
          // 下载/替换失败也要把内核拉回来，别把用户停在「没有内核」的状态。
          void ensureKernel().catch(() => {})
          throw error
        }
        const instance = await ensureKernel()
        sendJson(res, 200, { ok: true, bytes, models: await instance.models() })
      })
    })
    reg({
      kind: 'exact',
