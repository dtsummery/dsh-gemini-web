...
const Config = z.object({
   ...
   *  默认 1 分钟——Google 轮转后旧票立即失效，间隔太长会出现几分钟的失败窗口。 */
  keepAliveIntervalMinutes: z.number().default(1),
...
function normalizeConfig(...sources) {
  ...
  return {
    ...
    keepAliveIntervalMinutes: Number.isFinite(keepAliveMinutes) ? Math.min(120, Math.max(1, Math.floor(keepAliveMinutes))) : 1,
