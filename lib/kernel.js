...
export class Kernel {
  ...
  /**
   * 重启内核：托管实例直接结束子进程；复用来的残留实例按监听端口反查 PID 再结束，
   * 否则「重启内核」在复用场景下会静默失效（杀不掉进程又立刻复用它）。
  ...
  /**
   * Cookie 池自愈守护：内核访问上游失败累计 3 次后会把该账号标成 disabled，
   * 此后所有请求都报「cookie 池里没有可用账号」，看起来像 cookie 过期，实际多半是
   * 出口抖动把账号熔断了。这里定期检查被禁用的账号，检测通过就重新启用；
   * 真失效才提示重新登录。启动内核后调用，生命周期与内核一致。
   */
  startCookieGuard({ intervalMs = 600000, onLog } = {}) {
    if (this.guardTimer) return
    const tick = async () => {
      if (!this.running) return
      try {
        const pool = await this.listCookies()
        for (const item of pool.items ?? []) {
          if (item.status === 'enabled') continue
          const check = await this.checkCookie(item.id).catch(() => undefined)
          if (check?.ok) {
            await this.toggleCookie(item.id).catch(() => {})
            onLog?.(`[guard] #${item.id} cookie 有效但被禁用 → 已重新启用`)
          } else if (check) {
            onLog?.(`[guard] #${item.id} cookie 已失效（${check.detail ?? check.error ?? '未知原因'}）—— 需要重新登录`)
          }
        }
      } catch (error) {
        onLog?.(`[guard] 检查出错：${String(error?.message ?? error)}`)
      }
    }
    this.guardTimer = setInterval(tick, intervalMs)
