...
/**
 * 内核真实提供的模型目录（与 gemini-web2api-go v4.20.0 的 `/v1/models` 对齐）。
 *
 * 这里只定义「可选清单 + 展示名 + 输入能力」；**实际暴露哪些给 DSH 由设置页勾选**
 * （配置项 `models`），没勾选的不进入会话窗口的模型选择器。
 * 思考版与生图 / 音乐 / 画布 / 视频都需要有效登录态，内核在 cookie 失效时会直接报错，
 * 不会静默降级成匿名模型。
 */
export const MODEL_CATALOG = [
  { id: 'gemini-3.8-flash', name: 'Gemini 3.8 Flash', input: ['text', 'image'] },
  { id: 'gemini-3.8-flash-thinking', name: 'Gemini 3.8 Flash · 扩展思考', input: ['text', 'image'] },
  { id: 'gemini-3.7-flash', name: 'Gemini 3.7 Flash', input: ['text', 'image'] },
  { id: 'gemini-3.7-flash-thinking', name: 'Gemini 3.7 Flash · 扩展思考', input: ['text', 'image'] },
  { id: 'gemini-3.6-flash', name: 'Gemini 3.6 Flash', input: ['text', 'image'] },
  { id: 'gemini-3.6-flash-thinking', name: 'Gemini 3.6 Flash · 扩展思考', input: ['text', 'image'] },
  { id: 'gemini-3.5-flash-lite', name: 'Gemini 3.5 Flash Lite', input: ['text', 'image'] },
  { id: 'gemini-3.5-flash-lite-thinking', name: 'Gemini 3.5 Flash Lite · 扩展思考', input: ['text', 'image'] },
  { id: 'gemini-3.1-pro', name: 'Gemini 3.1 Pro', input: ['text', 'image'] },
  { id: 'gemini-3.1-pro-thinking', name: 'Gemini 3.1 Pro · 扩展思考', input: ['text', 'image'] },
  { id: 'gemini-image', name: 'Gemini 生图（Nano Banana）', input: ['text'] },
  { id: 'gemini-music', name: 'Gemini 音乐（Lyria）', input: ['text'] },
  { id: 'gemini-canvas', name: 'Gemini 画布', input: ['text'] },
  { id: 'gemini-video', name: 'Gemini 视频（Veo，需 Pro 号）', input: ['text'] }
]

/** 默认暴露给 DSH 的模型（设置页可改，改完保存即写回配置）。 */
export const DEFAULT_MODELS = ['gemini-3.8-flash', 'gemini-3.1-pro']
...
export class GeminiWebAdapter extends LlmAdapter {
  ...
  async *stream(options) {
    ...
    if (!res.ok || !res.body) {
      ...
      // 内核报「cookie 池不可用 / no SNlM0e / /app 302」＝ 手里这张登录票刚被 Google 轮转掉。
      // 主动催一次保活，把浏览器里的新票回写内核，用户重发就能成功。
      const staleTicket = /cookie 池|no SNlM0e|fetch \/app: HTTP 302/.test(text)
      if (staleTicket) this.deps.onAuthFailure?.(text)
      const code = res.status === 401 || res.status === 403
        ? 'AUTH'
        : res.status === 429 ? 'RATE_LIMIT' : res.status >= 500 ? 'SERVER' : 'INVALID_REQUEST'
      throw new LlmError(
        `Gemini 反代返回 HTTP ${res.status}：${text.slice(0, 500)}${staleTicket ? '（登录票刚被 Google 轮转，已自动刷新，重发一次即可）' : ''}`,
        code
      )
    }
