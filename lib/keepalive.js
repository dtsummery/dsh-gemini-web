...
import { CdpClient, browserProcessName, detectBrowser } from './edge-login.js'
...
export class KeepAliveBrowser {
  ...
  async start() {
    ...
    await this.stop()
    // DSH 被强杀时，上一轮的 headless 实例会活下来占住同一个 profile，
    // 新实例随后起不来（单实例锁 + 端口文件），所以先把占坑的清掉。
    await this.killStaleProfileProcesses()
  ...
  /**
   * 结束仍占着本 profile 的**残留 headless** 实例。
   *
   * 只匹配「命令行里带本 profile 路径」且带 `--headless` 的进程：
   * 保活浏览器永远跑 headless，而登录窗口是可见窗口 —— 多这一层过滤，
   * 就不会在竞态里把用户正在操作的登录窗口一并杀掉。
   * @returns {Promise<boolean>} 是否执行了清理命令（非 Windows 返回 false）。
   */
  async killStaleProfileProcesses() {
    if (process.platform !== 'win32') return false
    const name = browserProcessName(detectBrowser(this.opts.browserPath))
    const script = `Get-CimInstance Win32_Process -Filter "Name='${name}'" | Where-Object { $_.CommandLine -like '*${this.opts.profileDir}*' -and $_.CommandLine -like '*--headless*' } | ForEach-Object { Stop-Process -Id $_.ProcessId -Force -ErrorAction SilentlyContinue }`
    await new Promise((resolve) => {
      execFile('powershell', ['-NoProfile', '-NonInteractive', '-Command', script], { windowsHide: true, timeout: 20000 }, () => resolve(undefined))
    })
    await sleep(800)
    return true
  }
