...
export class KeepAliveBrowser {
  ...
  /**
   * 结束仍占着本 profile 的残留浏览器进程。
   * 只匹配命令行里带本 profile 路径的实例，不会碰到用户自己的浏览器窗口。
  ...
  async killStaleProfileProcesses() {
    ...
    const script = `Get-CimInstance Win32_Process -Filter "Name='${name}'" | Where-Object { $_.CommandLine -like '*${this.opts.profileDir}*' } | ForEach-Object { Stop-Process -Id $_.ProcessId -Force -ErrorAction SilentlyContinue }`
